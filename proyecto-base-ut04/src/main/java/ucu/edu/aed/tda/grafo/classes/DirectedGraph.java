package ucu.edu.aed.tda.grafo.classes;

import ucu.edu.aed.tda.grafo.algorithms.GraphAlgorithms;
...
public class DirectedGraph<V, D> implements IDirectedIGraph<V, D> {
    ...
    public boolean eliminarArista(Comparable<V> source, Comparable<V> target) {
        ...
        edges.stream().filter(x -> source.compareTo(x.source()) == 0 && target.compareTo(x.target()) == 0)
                .forEach(removeThis::add);
    ...
    public boolean removerVertice(Comparable<V> criteria) {
        ...
        edges.stream().filter(x -> criteria.compareTo(x.source()) == 0 || criteria.compareTo(x.target()) == 0)
                .forEach(removeThisE::add);
    ...
    public boolean existeArista(Comparable<V> sourceCriteria, Comparable<V> targetCriteria) {
        return edges.stream()
                .anyMatch(x -> sourceCriteria.compareTo(x.source()) == 0 && targetCriteria.compareTo(x.target()) == 0);
    }
    ...
    public Edge<V, D> obtenerArista(Comparable<V> sourceCriteria, Comparable<V> targetCriteria) {
        return edges.stream()
                .filter(x -> sourceCriteria.compareTo(x.source()) == 0 && targetCriteria.compareTo(x.target()) == 0)
